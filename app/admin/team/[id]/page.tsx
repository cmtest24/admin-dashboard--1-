...
export default function TeamMemberEditPage() {
  ...
  const memberId = isNew ? null : (params.id as string)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = isNew ? "/api/team" : `/api/team/${memberId}`
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || "Không thể lưu thông tin thành viên")
      }

      toast({
        title: "Thành công",
        description: isNew ? "Đã thêm thành viên mới" : "Đã cập nhật thông tin thành viên",
      })
    ...
    <AdminLayout>
      <div className="container max-w-4xl py-8">
        <div className="mb-6 flex items-center">
          ...
          <h1 className="ml-4 text-2xl font-bold">{isNew ? "Thêm thành viên mới" : "Chỉnh sửa thành viên"}</h1>
        </div>
